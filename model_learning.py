...
def auc_score(model, X_trn, y_trn, X_tst, y_tst, has_predict_proba=True):
	...
	if has_predict_proba:
		...
		pp_trn = model.decision_function(X_trn)+1
		pp_tst = model.decision_function(X_tst)+1

	auc_trn = roc_auc_score(y_trn, pp_trn)
	auc_tst = roc_auc_score(y_tst, pp_tst)


	print "Train AUC = %.3f, Test AUC = %.3f" % (auc_trn, auc_tst)

	# p_trn = model.predict(X_trn)
	# p_tst = model.predict(X_tst)

	scr_trn = model.score(X_trn, y_trn)
	scr_tst = model.score(X_tst, y_tst)

	print "Train score = %.3f, Test score = %.3f" % (scr_trn, scr_tst)
...
def xgbfit(alg, dtrain, dtest, predictors, 
    ...
    dtrain_pred = alg.predict_proba(dtrain[predictors])[:, 1]
