...
def Ztest_bi(df, cate_col, nume_col):
	means = df.groupby(cate_col)[nume_col].mean()
	stds = df.groupby(cate_col)[nume_col].std()
	counts = df.groupby(cate_col).size()
...
## OvA 
def Ztest(df, cate_col, nume_col):
	gsize_thresh = 20

	cate_values = df[cate_col].unique()
	group_z_stats = list()
	group_size = list()
	...
	for cv in cate_values:
		if str(cv) != 'nan':
			g1 = df.loc[df[cate_col] == cv, nume_col]
			g2 = df.loc[df[cate_col] != cv, nume_col]
		else:
			g1 = df.loc[df[cate_col].isnull(), nume_col]
			g2 = df.loc[~df[cate_col].isnull(), nume_col]
