...
def CreateCateFreqColumns(trn, tst, cate_columns):
	trn['train'] = 1
	tst['train'] = 0
	...
	for c in cate_columns:
		...
		df_freq = df_all.groupby(c).size() / N
		df_freq = df_freq.reset_index()
		df_freq.columns = [c, new_c]

		df_all = pd.merge(df_all, df_freq, how='left', on=c)
...
def CreateRescaledColumns(trn, tst, in_columns):
	out_columns = ['rescaled_' + c for c in in_columns]

	scaler = StandardScaler()
	scaler = scaler.fit(trn[in_columns])

	trn_mat = scaler.transform(trn[in_columns])
	trn_new = pd.DataFrame(trn_mat, columns=out_columns, index=trn.index)
	trn = pd.concat([trn, trn_new], axis=1)

	tst_mat = scaler.transform(tst[in_columns])
	tst_new = pd.DataFrame(tst_mat, columns=out_columns, index=tst.index)
	tst = pd.concat([tst, tst_new], axis=1)

	return trn, tst, out_columns
...
def CreateSmoothingColumns(df_trn, df_tst, in_columns, target, min_samples_leaf=200, smoothing=10, noise_level=0, verbose=True):
	new_columns = list()
	tgt_c = target
...
def CreateOOFColumns(df_train, df_test, categorical_columns, target=target, cv=None, noise_level=0, verbose=True):
	...
	for i, cate_c in enumerate(categorical_columns):
		...
		df_trn_new[new_c] = add_noise(df_trn_new[new_c], noise_level)
		df_tst_new[new_c] = add_noise(df_tst_new[new_c], noise_level)
		new_columns.append(new_c)
